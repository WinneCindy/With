...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if(self)
    {
        ...
        self.backgroundColor = getUIColor(Color_black);
...
{
    //set the key titles
    NSArray *titleArray = [[NSArray alloc] initWithObjects:@"1", @"2", @"3", @"4", @"5", @"6", @"7", @"8", @"9", @"删除", @"0", @"完成", nil];
...
- (void)onClick:(UIButton *)button
{
    ...
    if([button.currentTitle isEqualToString:@"删除"] && self.string.length > 0)
    {
        [self.string deleteCharactersInRange:NSMakeRange(self.string.length-1, 1)];
    }
    else if (![button.currentTitle isEqualToString:@"删除"] && ![button.currentTitle isEqualToString:@"完成"])
    {
