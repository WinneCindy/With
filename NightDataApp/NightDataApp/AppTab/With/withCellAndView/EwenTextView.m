...
- (void)keyboardWillShow:(NSNotification *)aNotification
{
    ...
    if (self.textView.text.length == 0) {
        ...
        self.backGroundView.frame = CGRectMake(0, kScreenheight-height-65, kScreenwidth, 65);
    }else{
...
{
    if (self.textView.text.length == 0) {
        self.backGroundView.frame = CGRectMake(0, 0, kScreenwidth, 65);
...
- (void)textViewDidChange:(UITextView *)textView{
    ...
    if (textView.text.length == 0) {
        self.placeholderLabel.text = placeholderText;
//        [self.sendButton setBackgroundColor:Color_Gold];
        
        [self.sendButton setImage:[UIImage imageNamed:@"cantSend"] forState:UIControlStateNormal];
        self.sendButton.userInteractionEnabled = NO;
    }else{
        self.placeholderLabel.text = @"";
//        [self.sendButton setBackgroundColor:Color_Gold];
         [self.sendButton setImage:[UIImage imageNamed:@"canSend"] forState:UIControlStateNormal];
        self.sendButton.userInteractionEnabled = YES;
    }
    
    //---- 计算高度 ---- //
    CGSize size = CGSizeMake(textView.width-10, CGFLOAT_MAX);
    ...
    if (curheight < 16.8) {
        statusTextView = NO;
        self.backGroundView.frame = CGRectMake(0, y - 65, kScreenwidth, 65);
    }else if(curheight < 50){
        statusTextView = NO;
        self.backGroundView.frame = CGRectMake(0, y - textView.contentSize.height-15, kScreenwidth,textView.contentSize.height+15);
    }else{
        statusTextView = YES;
        return;
    }
...
- (UIView *)backGroundView{
    if (!_backGroundView) {
        _backGroundView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, kScreenwidth, 65)];
        _backGroundView.backgroundColor = [UIColor whiteColor];
//        [_backGroundView.layer setCornerRadius:3];
//        [_backGroundView.layer setMasksToBounds:YES];
        
        UIBezierPath *maskPath = [UIBezierPath bezierPathWithRoundedRect:_backGroundView.bounds byRoundingCorners:UIRectCornerTopLeft | UIRectCornerTopRight cornerRadii:CGSizeMake(10, 10)];
...
- (UITextView *)textView{
    if (!_textView) {
        ...
        [_textView.layer setBorderColor:Color_white.CGColor];
        [_textView.layer setBorderWidth:1];
        [_textView setBackgroundColor:[UIColor clearColor]];
        _textView.font = [UIFont systemFontOfSize:14];
        _textView.delegate = self;
        [_textView setTextColor:getUIColor(Color_black)];
        [_textView setTintColor:getUIColor(Color_black)];
        _textView.layer.cornerRadius = 37.0 / 2;
...
//        [_sendButton setBackgroundColor:Color_Gold];
//        [_sendButton setTitle:@"发送" forState:UIControlStateNormal];
//        [_sendButton setTitleColor:getUIColor(Color_black) forState:UIControlStateNormal];
        [_sendButton setImage:[UIImage imageNamed:@"cantSend"] forState:UIControlStateNormal];
...
//        _sendButton.layer.cornerRadius = 33.0 /2 ;
